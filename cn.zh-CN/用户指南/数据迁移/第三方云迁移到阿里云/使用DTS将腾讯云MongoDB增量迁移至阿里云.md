...
-   腾讯云MongoDB副本集实例的数据库版本为3.6版本。
    ...
    **说明：** 关于3.2版本的腾讯云MongoDB副本集实例和分片集群实例的迁移方法，请参见[使用MongoDB工具将腾讯云MongoDB迁移至阿里云](cn.zh-CN/用户指南/数据迁移/第三方云迁移到阿里云/使用MongoDB工具将腾讯云MongoDB迁移至阿里云.md#)。
...
## 费用说明 {#section_jw7_hfd_x4t .section}

|迁移类型|链路配置费用|公网流量费用|
|----|------|------|
|全量数据迁移|不收取|不收取|
|增量数据迁移|收取，费用详情请参见[DTS产品定价](https://help.aliyun.com/document_detail/117780.html)。|不收取|

## 迁移类型说明 {#section_ao0_iyl_s02 .section}
...
|实例类型|全量数据迁移|增量数据迁移|
|:---|:-----|:-----|
|腾讯云MongoDB副本集实例|read权限| -   待迁移库的read权限
-   admin的read权限
-   local的read权限
...
2.  进入腾讯云服务器控制台，查看腾讯云服务器的内网IP地址与公网IP地址。
    ...
    ![查看云服务器的内外网IP地址](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/84333/156092088035509_zh-CN.png)

3.  进入腾讯云MongoDB控制台，查看腾讯云MongoDB副本集实例的内网IP地址。

    ![查看MongoDB实例的内网IP地址](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/84333/156092088035670_zh-CN.png)
...
8.  在入站规则页签，单击**添加规则**，放通MongoDB数据库端口27017，允许外网访问该端口。
    ...
    ![安全组配置](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/84333/156092088035723_zh-CN.png)

9.  进入腾讯云MongoDB控制台，单击目标MongoDB实例名。
10. 单击安全组页签，并单击**配置安全组**。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/519067/156092088049246_zh-CN.png)

11. 在弹出的配置安全组对话框，选择已放通27017端口的安全组，并单击**确定**。

    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/519067/156092088049247_zh-CN.png)
...
4.  配置迁移任务的**源库及目标库**信息。
    ...
    ![源库及目标库配置](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/84333/156092088136005_zh-CN.png)
...
6.  选择**迁移对象**和**迁移类型**。
    ...
    ![迁移对象和迁移类型配置](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/519067/156092088149320_zh-CN.png)
...
**说明：** 为保障数据一致性，全量数据迁移期间请勿在腾讯云MongoDB数据库中写入新的数据。
 ...
 |
    |迁移对象|     -   在**迁移对象**框中将想要迁移的数据库选中，单击![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/83046/156092088137966_zh-CN.png)移动到**已选择对象**框。
...
7.  上述配置完成后，单击页面右下角的**预检查并启动**。
    ...
    **说明：** 
    ...
    -   如果预检查失败，单击具体检查项后的![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/86903/156092088135996_zh-CN.png)，查看失败详情。根据失败原因修复后，重新进行预检查。
8.  预检查通过后，单击**下一步**。
...
10. 单击**购买并启动**，迁移任务正式开始。
    ...
    **说明：** 请选择合适的时间手动结束迁移任务，例如业务低峰期或准备将业务切换至MongoDB实例时。
    ...
    2.  等待增量迁移再次进入**增量迁移无延迟**状态，手动停止迁移任务。
        ...
        ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/519067/156092088249249_zh-CN.png)

11. 业务切换至阿里云MongoDB实例。
