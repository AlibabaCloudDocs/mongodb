# 将ECS上分片集群架构的自建MongoDB迁移上云 {#concept_1340083 .concept}
...
## 迁移原理介绍 {#section_u3c_tyw_ll7 .section}
...
![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/80861/156879123150227_zh-CN.png)
...
## 注意事项 {#section_n0l_yl7_0jx .section}
...
## 迁移类型说明 {#section_tpc_qd2_t7b .section}
...
在正式开始数据迁移之前，您需要关闭源数据库的均衡器并清理孤立文档，详情请参见[分片集群架构的数据库迁移前准备工作](../cn.zh-CN/分片集群快速入门/数据迁移/使用DTS迁移分片集群架构的自建MongoDB数据库上云.md#section_pcx_yhs_zfb)。
...
3.  在迁移任务列表页面顶部，选择阿里云MongoDB实例所属的地域。
    ...
    ![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/6682/156879123150190_zh-CN.png)
...
7.  选择迁移对象及迁移类型。
    ...
    ![MongoDB迁移对象迁移类型选择](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/123029/156879123238566_zh-CN.png)
...
**说明：** 为保障数据一致性，全量数据迁移期间请勿在自建MongoDB数据库中写入新的数据。
 ...
 |
    |迁移对象|     -   在**迁移对象**框中单击待迁移的对象，然后单击![](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/79929/156879123240698_zh-CN.png)将其移动到**已选择对象**框。
...
8.  上述配置完成后，单击页面右下角的**预检查并启动**。
    ...
    **说明：** 
    ...
    -   如果预检查失败，单击具体检查项后的![提示](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/140110/156879123250068_zh-CN.png)，查看失败详情。根据失败原因修复后，重新进行预检查。
9.  预检查通过后，单击**下一步**。
    ...
    -   增量数据迁移
        ...
        **说明：** 请选择合适的时间手动结束迁移任务，例如业务低峰期或准备将业务切换至MongoDB实例时。
        ...
        2.  等待所有Shard节点迁移任务的**增量迁移**再次进入**无延迟**状态后，手动结束迁移任务。
            ...
            ![结束迁移任务](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/80861/156879123234689_zh-CN.png)
